...
namespace Drivocopter {
    ...
    float R12DS::parse_signal(uint32_t value) {
        return ((float) value - 1500.0F) / 500.0F;
    }

    uint8_t R12DS::parse_mode(uint32_t value, bool type) {
        if (type) {
            if (value > 1500) {
                return 0;
            }
